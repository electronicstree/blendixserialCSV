...
coordinate sets. The receiver extracts the first set's 9 values
(v0 .. v8) and uses v2 to position a servo motor, printing the
updated position over a separate debug serial connection whenever
it changes.
...
void setup() {
  ...
  blendix.setRxSets(1); // Set expected number of received sets
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (blendix.parseReceivedData(serialData)) {
      processCoordinates(); // Extract value and use it
...
void processCoordinates() {
  ...
  if (numSets > 0) { // Ensure there's at least one set of data
    ...
    if (blendix.getReceivedCoordinates(0, v0, v1, v2, v3, v4, v5, v6, v7, v8)) { // Get first object's values
      // v2 takes the place of the old "z" value 
      int servoPos = constrain(v2, 0, 180); // Ensure it's within the servo's range
